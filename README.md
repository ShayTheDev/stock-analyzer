## Ultimate Stock Analyzer 

A data science project build to analyze financial markets
and filter out systemic market panic from true, company specific signals.

## Current Milestones
* [x] Set up Git & GitHub repository architecture
* [] Import historical stock data arrays using NumPy
* [] Apply multi-dimensional array slicing to isolate specifc trading windows
* [] Implement core statistical metrics (mean, volatility, standard deviation)  